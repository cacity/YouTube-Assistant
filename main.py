...
# 导入自定义模块
import config
from utils.downloader import download_youtube_video, extract_audio_from_video, download_youtube_batch
from utils.subtitle_extractor import transcribe_audio_to_text, create_subtitles, create_bilingual_subtitles, embed_subtitles_to_video
from utils.translator import translate_text
from utils.summarizer import save_summary
from utils.composite import TextSummaryComposite
from utils.common import ensure_directory, read_text_from_file
...
def process_youtube_videos_batch(youtube_urls, model=None, api_key=None, base_url=None, whisper_model_size="tiny", 
                                stream=True, summary_dir=None, download_video=False, generate_subtitles=True, 
                                translate_to_chinese=True, embed_subtitles=True):
    """
    批量处理多个YouTube视频
    ...
    for i, url in enumerate(youtube_urls):
        try:
            print(f"处理第 {i+1}/{len(youtube_urls)} 个YouTube视频: {url}")
            ...
            print(f"处理YouTube视频 {url} 时出错: {str(e)}")
